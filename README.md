...
- 悬停/点击换算结果，浮窗展示当前美元金额对应的多币种结果（欧元、英镑、日元、港币、新加坡元、韩元、澳元、加元），带国旗标识；滚动时浮窗跟随，不会误关
...
若某页面不希望被换算（例如本脚本的演示页自带模拟器，避免重复展示），在页面 `<head>` 加一行即可让脚本在该页整体退出：

```html
<meta name="ai-cost-helper" content="disabled">
```
...
- 不处理页面源码、输入框、代码块中的价格
- `$` 前缀以外的价格格式暂不支持
- 深色浮窗/面板跟随系统偏好（`prefers-color-scheme`），不适配站点自定义的暗色主题
